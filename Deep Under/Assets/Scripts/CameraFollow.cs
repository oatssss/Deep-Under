...
public class CameraFollow : MonoBehaviour {
	...
	void FixedUpdate () {
		...
		if (Mathf.Abs(Input.GetAxis("Mouse X")) > 0.1f)transform.RotateAround(transform.position, transform.up, Input.GetAxis ("Mouse X") * cameraSpeed * Time.deltaTime); //rotate camera on Y Axis according to MouseX
		if (Mathf.Abs(Input.GetAxis("Mouse Y")) > 0.1f)transform.RotateAround(transform.position, transform.right, Input.GetAxis ("Mouse Y") * cameraSpeed * Time.deltaTime); //rotate camera on X axis according to MouseY
    ...
    private void fixRoll(){
    	if (transform.rotation.eulerAngles.z > 2f && transform.rotation.eulerAngles.z < 358f) {
	    	if (transform.rotation.eulerAngles.z > 180f) { 
	    		transform.RotateAround(transform.position, transform.forward, Time.deltaTime * cameraSpeed/2f );
	    	}
	    	else {
	    	transform.RotateAround(transform.position, transform.forward, -Time.deltaTime * cameraSpeed/2f );
	    	}
